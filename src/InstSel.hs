...
-- FIXME: duplicación
cgExpr (BinOp (Lang.Plus _) v1 v2) = do
  v1 <- cgV v1
  v2 <- cgV v2
  vf1 <- freshName
  vf2 <- freshName
...
cgExpr (BinOp (Lang.Minus _) v1 v2) = do
  v1 <- cgV v1
  v2 <- cgV v2
  vf1 <- freshName
  vf2 <- freshName
...
cgExpr (UnOp Lang.Succ v) = do
  cgExpr (BinOp (Lang.Plus True) v (C 1)) -- trucho

cgExpr (UnOp Lang.Pred v) = do
  cgExpr (BinOp (Lang.Minus True) v (C 1)) -- trucho
...
-- truchísimo
cgExpr (V v) = do
  cgExpr (BinOp (Lang.Plus False) v (C 0))
