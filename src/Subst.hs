...
varChanger local bound t = go 0 t where
  ...
  go n (Lam p y ty t)   = Lam p y ty (go (n+1) t)        -- \x. x (\y. x y)  == \0 (\1 0)        
  ...
  go n t@(Const _ _) = t
  go n (BinaryOp p op t t') = BinaryOp p op (go n t) (go n t')
  go n (TLet p x ty t1 t2) = TLet p x ty (go n t1) (go (n+1) t2)
